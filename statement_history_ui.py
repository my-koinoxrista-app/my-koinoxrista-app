...
def render(building_id):
    ...
    st.caption(
        'Εκδοθείσες εκκαθαρίσεις. Τα παλιά PDF και ποσά δεν επανυπολογίζονται.'
    )
    ...
    try:
        ...
        st.error(
            ...
            'Ελέγξτε τη σύνδεση και τα migrations.'
        )
        return
    ...
    st.dataframe(
        pd.DataFrame(
            [
                {
                    'Περίοδος': s['period_key'],
                    'Έκδοση': s['revision'],
                    'Έκδοση στις (Αθήνα)': s['issued_at']
                    .astimezone(ZoneInfo('Europe/Athens'))
                    .strftime('%d/%m/%Y %H:%M'),
                    'Ιδιοκτησίες': s['property_count'],
                    'Σύνολο €': float(s['total']),
                }
                for s in statements
            ]
        ),
    ...
    chosen = st.selectbox(
        ...
        format_func=lambda s: (
            f"{s['period_key']} · Έκδοση {s['revision']} · "
            f"{s['issued_at'].astimezone(ZoneInfo('Europe/Athens')):%d/%m/%Y}"
        ),
    ...
    try:
        ...
        st.download_button(
            ...
            file_name=(
                ...
                f"v{statement['revision']}.pdf"
            ),
            mime='application/pdf',
            key=f"history_pdf_{statement['id']}",
        )

        section = st.segmented_control(
            'Ενότητα',
            ['Ατομικά PDF', 'Αποστολή email', 'Πληρωμές'],
            default='Ατομικά PDF',
            key=f'history_section_{building_id}_{statement["id"]}',
        )

        if section == 'Ατομικά PDF':
            ...
            st.dataframe(
                pd.DataFrame(
                    [
                        {
                            'Ιδιοκτησία': d['property']['code'],
                            'Όνομα / περιγραφή': d['property'].get('name', ''),
                            'Ένοικος': d['property'].get('tenant_name', ''),
                            'Ιδιοκτήτης': d['property'].get('owner_name', ''),
                            'Ενοίκου €': float(
                                d['property']['totals']['TENANT']
                            ),
                            'Ιδιοκτήτη €': float(
                                d['property']['totals']['OWNER']
                            ),
                            'Λοιπές €': float(
                                d['property']['totals']['OTHER']
                            ),
                            'Σύνολο €': float(d['property']['total']),
                        }
                        for d in documents
                    ]
                ),
            ...
            for d in documents:
                ...
                with st.expander(
                    ...
                    st.download_button(
                        ...
                        mime='application/pdf',
                        key=(
                            f"history_property_{statement['id']}_"
                            f"{d['apartment_id']}"
                        ),
                    )

        elif section == 'Αποστολή email':
            statement_delivery_ui.render(
                building_id,
                statement,
            )

        elif section == 'Πληρωμές':
            payment_ui.render(
                building_id,
                statement,
            )
